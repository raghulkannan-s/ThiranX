package com.raghul.thiranx.data.repository;
...
public class ThiranXDB {
    ...
    public boolean markNotificationRead(Long employeeId, Long notificationId) {
        if (employeeId == null || notificationId == null) return false;
        for (Notification current : notifications) {
            if (notificationId.equals(current.getId())
                    && employeeId.equals(current.getEmployeeId())) {
                if (!Boolean.TRUE.equals(current.getIsRead())) {
                    current.setIsRead(Boolean.TRUE);
                }
                return true;
            }
